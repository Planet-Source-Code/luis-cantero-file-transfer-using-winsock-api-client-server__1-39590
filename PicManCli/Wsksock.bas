...
'********************
'Modifications, improvements and additions © 2002 by Luis Cantero
'Modifications: ListenForConnect, ConnectSock, SendData, StartWinsock, IsConnected,
'16-Bit declarations removed.
'Additions: Ping, GetSMTPserver, MyIP, HTTPRequest, etc.
'L.C. Enterprises - http://LCenterprises.net
'********************
...
Function ConnectSock(ByVal host$, ByVal Port&, ByVal HWndToMsg&) As Long
  ...
  Dim sockin As sockaddr
    ...
    If WSAAsyncSelect(s, HWndToMsg, ByVal &H202, ByVal SelectOps) Then
        If s > 0 Then
            dummy = closesocket(s)
        End If
        ConnectSock = INVALID_SOCKET
...
Function GetSMTPserver(strEmail As String) As String
    ...
    Dim intCurrentPos As Integer
      ...
      'Convert Server: yahoo.com -> yahoo[3]com
...
Function HTTPRequest(strURL As String, strMethod As String, HWndToMsg As Long, Optional strProxy As String, Optional strPostCommand As String) As Long
  ...
  Dim strMsg As String
    ...
    If strProxy <> "" Then 'There is a Proxy
      ...
      Else 'No Proxy
        ...
        If intDot > 0 Then 'Port needs to be changed
    ...
    'Connect
    Sock = ConnectSock(strHost, intPort, HWndToMsg)
    ...
    If strMethod = "POST" Then 'Complete POST request
...
':) Ulli's VB Code Formatter V2.10.8 (16.04.2002 22:05:42) 313 + 965 = 1278 Lines
